...
it('documents the Workflow Studio editor and its non-mutating validation contract', function () {
    ...
    expect($integration)->toContain('eight routes')
        ...
        ->and($editor)->toContain('`Delete` / `Backspace`')
        ->and($editor)->toContain('`Cmd/Ctrl+K`')
        ->and($editor)->toContain('`Shift+L`')
        ->and($editor)->toContain('`F`')
        ->and($editor)->toContain('Shortcuts are suppressed while typing in')
        ->and($editor)->not->toContain('Cmd/Ctrl+Y')
        ->and($editor)->toContain('`urls.draft`, `urls.validate`, `urls.publish`, and `urls.options` are server-owned')
        ->and($editor)->toContain('at the drop point, nudged only when needed to avoid overlap')
