...
Only these terminal run statuses are eligible: `completed`, `failed`, and `cancelled`. `pending`, `running`, `waiting`, and `blocked` are preserved at every age. In particular, `blocked` may become recoverable after a missing node type is registered or aliased; automatic deletion would destroy the state needed to resume it.
