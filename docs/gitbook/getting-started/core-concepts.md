...
An **edge** connects one named output to the next node. For example, an `app.send_welcome` node can route its `sent` output to `core.exit`. A graph has one `start` node and must be valid before it can be published.
