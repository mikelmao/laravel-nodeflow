...
| `nodeflow.limits.max_steps_per_run` | `1000` | Integer-like value; it is cast to `int` when a run starts and is otherwise not validated. | None. | Caps interpreter node steps for each newly started engine workflow. Reaching the cap stops the loop without an explicit error, then the completion activity marks the run completed; subjects already advanced into an unprocessed cursor can remain active. `0` or a negative value completes immediately without executing a node. Use a positive value above the maximum valid graph depth, while retaining a loop guard. |
