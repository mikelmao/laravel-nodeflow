...
**One finding was reframed rather than accepted as stated.** The review held that
`tests/Unit/NodeRegistrationWriterTest.php`'s "leading backslash is optional" test asserts incorrect
PHP semantics. It does not — it asserts a deliberately lenient needle, and a host writing that form
already has broken code. But the substance underneath is real and worse than stated, and is recorded
as a **pre-existing shipped defect independent of this plan**: `appendTo()`'s presence check runs
`str_contains` over the whole comment-stripped file rather than the target array, so a mention
anywhere reads `AlreadyPresent`. **E50** absorbs the fix because it is already touching that code;
`open-issues.md` records it as found here.
...
| E40 | **G-10 is closed in code**, not re-documented. The bounded short-name matcher built for removal is shared with `appendTo()`'s presence check | Measured free: no existing test asserts the short-name duplicate behaviour, so all 16 writer tests stay untouched. The gap's documentation was deleted in Plan 5's Task 3 and spec §3.4 still cites it; closing it makes the citation correct to delete rather than restate |
...
| 4 | `NodeRemovalOutcome`, `removeFrom()`, and `appendTo()`'s array-bounded presence fix (**E38**, **E39**, **E40**, **E50**) | 16 writer tests untouched; the shipped whole-file defect proven fixed |
