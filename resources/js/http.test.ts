...
describe('send', () => {
    ...
    it('resolves with the status even when the body is not json', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response('<html>Page Expired</html>', { status: 419, headers: { 'Content-Type': 'text/html' } })))
    ...
    // Accept application/json prevents Laravel validation redirect. Counterfactual:
    // drop header and publish 422 arrives as 302.
    it('asks for json and sends the body as json', async () => {
        ...
        // Counterfactual: remove ...csrfHeaders() and isolated helper tests still
        // pass, but real writes 419. Pins integration.
...
describe('optionsUrl', () => {
    // Counterfactual: interpolate without encodeURIComponent and slash addresses different route.
    it('substitutes both sentinels, url-encoded', () => {
        ...
        expect(optionsUrl(template, 'yaya.send/message', 'template')).toBe('https://app.test/admin/flows/12/nodes/yaya.send%2Fmessage/fields/template/options')
    })
    // Missing sentinels is server contract change; counterfactual return unchanged causes mysterious 404.
