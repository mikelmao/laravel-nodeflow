...
import { ConfigPanel, nodeConfigFieldId } from './ConfigPanel'
...
export function NodeInspector({
    ...
    useEffect(() => {
        ...
        const field = document.getElementById(nodeConfigFieldId(node.id, selectedIssue.field!))
    ...
    return (
        <aside aria-label="Node inspector" className="flex min-h-0 flex-col gap-4 rounded-lg border border-border bg-card p-4 text-card-foreground">
            ...
            {activeTab === 'configure' ? (
                <div id={tabPanelId(generatedId, 'configure')} role="tabpanel" aria-labelledby={configureTabId} className="min-h-0 overflow-y-auto">
                    <ConfigPanel node={node} def={def} controls={controls} errors={errors} onConfigChange={onConfigChange} />
                </div>
            ) : (
                <div id={tabPanelId(generatedId, 'advanced')} role="tabpanel" aria-labelledby={advancedTabId} className="space-y-5">
