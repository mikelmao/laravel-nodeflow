...
function def(overrides: Partial<NodeTypePayload> = {}): NodeTypePayload {
    return { type: 'app.send', label: 'Send message', group: 'Messaging', icon: null, description: 'Sends one message', outputs: ['sent', 'failed'], fields: [], default_config: {}, cardinality: ['subject'], ...overrides }
}
...
const nodeProps: NodeProps<NodeflowNode> = { id: 'n1', data, type: 'nodeflowNode', selected: false, dragging: false, zIndex: 0, isConnectable: true, positionAbsoluteX: 0, positionAbsoluteY: 0, selectable: true, deletable: true, draggable: true }
...
describe('NodeCard', () => {
    // Handles belong to NodeCard. Counterfactual move to default renderer and host override loses wiring.
    it('draws one source handle per declared output even under a host renderer', () => {
        const Mine = () => <p>mine</p>
        const { container } = render(<ReactFlowProvider><CanvasContext.Provider value={{ defs: { 'app.send': def() }, renderers: { 'app.send': Mine }, nodeErrors: {} }}><NodeCard {...nodeProps} /></CanvasContext.Provider></ReactFlowProvider>)
    ...
    it('renders the errors recorded against its own id and no others', () => {
        ...
        render(<ReactFlowProvider><CanvasContext.Provider value={{ defs: { 'app.send': def() }, renderers: { 'app.send': Mine }, nodeErrors: { n1: ['field [template]: required'], n2: ['not mine'] } }}><NodeCard {...nodeProps} /></CanvasContext.Provider></ReactFlowProvider>)
...
describe('interactionProps', () => {
    // Global false flags alone leave keyboard/select/delete. Counterfactual omit any and run graph acts editable.
    it('turns every graph mutation and selection affordance off for a read-only canvas', () => {
        expect(interactionProps(false)).toEqual({ nodesDraggable: false, nodesConnectable: false, nodesFocusable: false, edgesFocusable: false, elementsSelectable: false, edgesReconnectable: false, deleteKeyCode: null, disableKeyboardA11y: true })
        const behavior = canvasBehavior(false, [{ ...canvasNode, draggable: true, selectable: true, deletable: true, focusable: true, connectable: true }], [{ ...canvasEdge, selectable: true, deletable: true, focusable: true, reconnectable: true }], { onNodesChange: vi.fn(), onEdgesChange: vi.fn(), onConnect: vi.fn() })
...
describe('Canvas', () => {
    // Mount smoke only; jsdom cannot measure layout. Counterfactual forget nodeTypes registration and label absent.
    it('mounts and renders a node through the registered node type', () => {
        render(<Canvas nodes={[canvasNode]} edges={[]} defs={{ 'app.send': def() }} />)
        expect(screen.getByText('Send message')).toBeInTheDocument()
    })
    ...
    it('applies read-only mode to the mounted nodes, handles, callbacks and keyboard path', () => {
        ...
        const { container } = render(<Canvas interactive={false} nodes={[{ id: 'n1', type: 'nodeflowNode', position: { x: 0, y: 0 }, data, selected: true, draggable: true, selectable: true, deletable: true, focusable: true, connectable: true }]} edges={[]} defs={{ 'app.send': def() }} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} />)
        expect(screen.getByTestId('rf__node-n1')).not.toHaveAttribute('tabindex')
        for (const handle of container.querySelectorAll('.react-flow__handle')) expect(handle).not.toHaveClass('connectable')
